...
- 曲线选择会持久保存；重新进入 Project 或 Run 时会自动恢复选择并重新请求历史数据，不需要再次选择。
- 已显示的曲线卡片可向左拖动；达到展开阈值后露出红色“确认删除”，只有再次点击确认才会移除。该操作只取消本地显示和选择，不会删除 W&B 指标或 Run。
- Runs 列表和 Run 详情的时间统一转换为 `yyyy-MM-dd HH:mm:ss 北京时间`。
- 首次读取只建立 Run 状态基线；此后 Run 进入 `finished`/`completed` 时在 Android 通知栏提醒。登录后会立即检查一次，前台与后台检查均会去重。
